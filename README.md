...
在第三种方式中，设想这样一个场景，父组件向子组件A传递变量num，向子组件B传递改变变量的方法updateNum，且父组件自身也使用到了num，那么在B中每次修改num值，都会造成父组件的重新渲染。  
在这种场景下，从组件B的角度上来看，他只是接受了一个函数，按道理来说应该不会每次都次重新渲染。但由于我们没有组件的渲染进行任何优化，所以每次父组件重新渲染，传到组件B的props每次都是一个新的props，因此每次都会造成组件B也重新渲染。解决方法就是：如果使用mobx管理数据的话，用observer包裹组件B(内部已经使用了React.memo)，或者直接用Reat.memo包裹组件B。  
有一个地方需要注意的是，如果从父组件中传递给了子组件B一个不变的函数，并且子组件用observer/memo包裹了，父组件重新渲染，子组件是不会重新渲染的。不变的函数是指useState这种产生的函数，因为hook是保存在fiber上的，每次都是取的fiber上同一个改变的方式，所以这个是一个不变的函数。但如果还有一些额外的操作需要进行，比如将这个setState放在handleClick中，然后进行了一些其他操作，最后将这个handleClick传给子组件B，那么父组件的重新渲染是会导致B重新渲染的，就算B包裹了memo或者observer都没有用，因为父组件每次重新渲染都的确会产生一个新的handleClic，对B来说这是一个新的props。
