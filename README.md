...
### 怎么理解按值传递  
参数如果是基本数据类型，那就是按值传递，就是把这个值从一个变量复制给另一个变量。  
如果传递的参数是一个对象，其实是共享传递，共享传递是指传递的是这个对象的引用的一个副本，由于从引用到引用的副本也是一种值的拷贝，所以也算是按值传递。  
```
var obj = {
  value = 1
};
...
```  
在上面的例子中就是一种对象的共享传递，第一种直接改变了原来的obj中的value，是因为可以通过这个对象引用的副本，访问原来的引用，达到修改的目的。但是在第二个例子中相当于直接扔掉了这个副本，给函数内的变量指定了一个新的内存地址，所以也就不会修改原对象中的值。  
...
异步加载的原理
异步加载本质上分成三个步骤：
1、当我们触发某个功能的时候，就去加载那个异步的文件
2、获取到这个异步的文件之后，将这个文件和 引用这个文件的文件 合并到一起
3、正式加载和解析这个异步文件的内容
...
在执行一段可执行代码(全局代码/函数代码/eval)的时候，就会创建一个执行上下文，并将其压入执行上下文栈中，等函数执行完毕就会将其从栈中弹出。  
...
### 闭包  
闭包是指那些能够访问自由变量的函数。所以闭包是由两部分组成：函数+自由变量。自由变量是指，既不是函数参数，也不是函数局部变量的变量。但是从实践角度上讲，闭包是指函数访问了自由变量，并且即使创建自由变量的上下文已经销毁了，该自由变量仍然存在，仍然能被函数访问到。  

### 手写call
手写call的过程中有下面几个关键点需要注意：  
1、用什么方法获取外面传进来的参数，因为arguments并不是一个严格的数组类型，所以先要将其转成一个数组，再用slice截取外面传进来的参数部分。  
...
### 事件循环  
首先有几个相关概念：  
浏览器的渲染进程下面包含几个线程，分别是：  
GUI渲染线程：负责页面的渲染、布局和绘制  
JS引擎线程：负责解析和执行JS脚本  
事件触发线程：用来控制事件循环，比如鼠标点击，setTimeout之类的，在满足触发条件的时候，将事件加入到执行队列中  
定时器线程：setTimeout和setInterval所在的线程  
异步http请求线程：用于处理ajax请求，当请求完成，如果有回调，会通知事件触发线程  
其中GUI渲染线程和JS引擎线程互斥，因为后者可以操作DOM，如果渲染页面的同时操作DOM，那么渲染前后获取的元素可能不一致了  

再到事件循环中，JS中的任务分为同步任务和异步任务，同步任务是在JS引擎中执行的，他会被放入执行栈中，异步任务由事件触发线程管理，该线程维护了一个任务队列，异步任务的触发条件达成的时候，会将异步任务的回调加入到任务队列中。执行栈中的所有同步任务执行完毕之后，会去任务队列中读取可以执行的回调函数，将其加入到到执行栈中执行。  

所以当我们遇到setTimeout或者ajax之类的代码，实际的流程其实是：setTimeout/ajax这些代码主体其实是同步代码，只是js引擎线程会给定时器线程/异步请求线程发送通知，再等待多长时间之后将回调函数添加到任务队列中/在拿到请求的返回之后添加回调函数到任务队列中。任务队列中的任务要等到执行栈中的任务被执行完毕之后才会有机会被读取执行。  

每次执行栈中执行的代码相当于一个宏任务，比如主模块代码，setTimeout/setInterval这些都是宏任务，当前宏任务执行完毕之后，会立即执行宏任务执行期间产生的所有微任务，比如promise，process.nextTick()，然后再执行GUI渲染线程。然后再开启下一轮宏任务，这就是事件循环。  

### 垃圾回收  
在V8引擎中会根据变量对象存活时间长短采用不同的垃圾回收策略  
对于存活时间较短的变量，通常采用新生代策略。大多数对象刚开始都会被分配在这里，这块区域比较小，但是垃圾回收很频繁，是一种用空间换时间的方法。  
新生代的内存区间会被分为两块，分别称为From空间和To空间，变量最开始会被放在From空间中，当垃圾回收的时候，如果From空间中还有存活的对象，会被复制到To空间中保存，非存活对象会被回收掉，然后之前的To空间就转为From空间，完成From和To的对换。  
如果一个对象已经经历过这样的一次对换，或者To空间的内存占用率已经超过25%，就会将变量采用老生代的策略。这个过程称为变量晋升。  
老生代策略中，会采用标记清除和标记整理两种方式来进行处理。标记清除分为标记和清除两个阶段，首先会遍历堆内存中的所有对象，并且标记活着的对象，然后在清除阶段清理那些非活跃对象。这样处理之后堆内存内并不连续，如果想要存储大内存变量就很浪费空间，标记整理就是用来解决这种内存碎片化的问题。在回收过程将死亡的对象清除之后，会将剩余的活跃对象移动到堆内存空间的另一端，然后清理掉边界之外的其余所有内存。  
...
2、在class类里面，在constructor中用this.name = name,this.func = function () {},等方式声明定义的变量和方法，和不通过this这种类型声明定义的类型方法，在使用上有什么区别？
